...
const CoreTeam = () => {
  return (
    <div className="bg-primary-black overflow-x-hidden color1 min-h-screen flex flex-col relative">
      {/* ✅ Background Theme */}
      <div className="gradient-03 absolute top-0 left-0 w-full h-full z-0"></div>
      ...
      <div className="flex-grow flex flex-col items-center text-white px-4 md:px-8 w-full max-w-7xl mx-auto relative z-10">
                    ...
                    {department.members.map((member, idx) => (
                      <div key={idx} className="flex flex-col items-center">
                        <img
                          src={member.imgUrl}
                          alt={member.name}
                          className="w-48 sm:w-48 md:w-56 lg:w-64 xl:w-64 h-64 sm:h-64 md:h-72 lg:h-80 xl:h-80 object-cover border-2 border-primary-black transition-transform duration-300 hover:scale-105 rounded-lg"
                        />
                        <h3 className="mt-2 text-lg sm:text-xl md:text-2xl font-bold text-white">{member.name}</h3>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {/* ✅ Devs of Website Section (Slideshow on Small Screens, Static on Large Screens) */}
        <h1 className="text-center text-3xl md:text-4xl font-bold mt-10 md:mt-14 mb-6 md:mb-10">
          Dev Team
        </h1>
              ...
              <Carousel.Item key={idx}>
                <div className="flex flex-col items-center">
                  <img
                    ...
                    className="w-48 sm:w-48 md:w-56 lg:w-64 xl:w-64 h-64 sm:h-64 md:h-72 lg:h-80 xl:h-80 object-cover border-2 border-primary-black transition-transform duration-300 hover:scale-105 rounded-lg"
                  />
                  <h3 className="mt-3 text-lg sm:text-xl md:text-2xl font-bold text-white">{member.name}</h3>
                </div>
              </Carousel.Item>
            ))}
          </Carousel>
        ...
        {/* ✅ Show Static Photos Side by Side on Larger Screens */}
